...
def scrape_js(url: str, selector: str = None):  # type:ignore
    ...
    """
    # Check if URL points to a downloadable file (CSV, PDF, etc.)
    if re.search(r'\.(csv|pdf|xlsx?|txt)(\?|$)', url, re.IGNORECASE):
        return f"Error: URL appears to be a file download ({url}). Use fetch_api tool instead."
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                if "Download" in str(e):
                    browser.close()
                    return f"Error: URL triggers a download. Use fetch_api tool for: {url}"
                raise
...
1. **Scrape the page** - Use scrape_js tool to extract the quiz question, instructions, and any embedded data
   - If scrape_js indicates the URL is a file download, use fetch_api instead
2. **Gather required data** - If the quiz references external APIs, PDFs, or CSV data:
   - Use fetch_api for API endpoints AND for CSV/data files (NOT scrape_js)
...
**Critical Rules:**
- Use fetch_api for CSV files, NOT scrape_js (scrape_js is for HTML pages only)
...
- scrape_js: For HTML webpages only (NOT for CSV/PDF/data files)
- fetch_api: For JSON/API endpoints AND for CSV/data files
...
def solve_quiz(quiz: Quiz):
    ...
    while current_url and iteration < max_iterations:
        iteration += 1
        attempts = 0
        print(f"\n=== Iteration {iteration}: {current_url} ===")
        ...
        while attempts < t:
            ...
            if res is None:
                ...
                attempts += 1
                if attempts < t:
                    print(f"Retrying... (attempt {attempts + 1}/{t})")
                continue
            
            try:
                data = QuizResponse.model_validate(res.json())
                print(f"Response: correct={data.correct}, url={data.url}, reason={data.reason}")
            except Exception as e:
            ...
            if data.correct:
                ...
                if hasattr(data, 'delay') and data.delay:
                    print(f"Waiting {data.delay} seconds before next quiz...")
                    time.sleep(data.delay)
                break
            else:
                print(f"✗ Answer was incorrect. Reason: {data.reason}")
                reason = data.reason
                attempts += 1
                
                # If server provides a new URL even on wrong answer, follow it
                if data.url and data.url != current_url:
                    print(f"Server provided new URL, moving to: {data.url}")
                    current_url = data.url
                    break
                
                if attempts < t:
                    print(f"Retrying with reason... (attempt {attempts + 1}/{t})")
        
        # Check exit conditions
        if attempts >= t and (not hasattr(data, 'correct') or not data.correct):  # type: ignore
            if not hasattr(data, 'url') or not data.url:  # type: ignore
                print(f"Max attempts reached for URL: {current_url}. Stopping.")
                return
        
        if current_url is None:
            print("Quiz completed successfully! No more URLs.")
            return
